            ...
            <div class="statbox widget box box-shadow">
                <div class="widget-content widget-content-area">                    
                        <div class="d-flex justify-content-between mb-2">
                            <h5>Caja Sucursal : {{ $sucursalNombre }} </h5>                       
                            <a href="javascript:void(0)" class="btn btn-warning"
                                wire:click="movimientoCaja('{{ $almacen_id }}')">
                                Mov. Caja
                            </a>
                        </div>                    
                            ...
                            <tbody>
                                @forelse ($cajas as $caja)
                                    <tr>
                                        ...
                                        <td>$
                                            {{ number_format($caja->monto_apertura + $caja->monto_ingreso - $caja->monto_egreso - $caja->monto_cierre, 0, ',', '.') }}
                                        </td>
                                        ...
                                        <td class="text-center">
                                            @if (Carbon\Carbon::parse($caja->fecha_apertura)->format('Y-m-d') == $this->fechaActual->format('Y-m-d') &&
                                                    $caja->estado == 1)
                                                ...
                                                <a href="javascript:void(0)" class="btn btn-info"
                                                    wire:click="egreso('{{ $caja->id }}')" title="Egreso">
                                                    <i class="fas fa-minus" aria-hidden="true"></i>
                                                </a>
                                            @endif
                                            @if ($caja->estado == 1)
                                                <a href="javascript:void(0)" class="btn btn-danger"
                                                    onclick="Confirm('{{ $caja->id }}')" title="Cerrar Caja">
